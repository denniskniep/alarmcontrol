...
class EmployeeStatusAsList extends Component {
  ...
  getOrganisationId() {
    return store.getCurrentOrganisationId();
  }
    ...
    return (
        <React.Fragment>
          ...
          <Query query={ORGANISATION_BY_ID}
                 ...
                 variables={{organisationId: this.getOrganisationId()}}>
                      ...
                      <Table responsive>
                        ...
                        {organisation.employees.map((e, index) => {
                          return (
                              <tr key={e.id}>
                                <td className={"dot-td"}>
                                  <span className={"dot dot-td-container " + this.mapClassForFeedback(
                                e.status && e.status.status)}></span>
                                </td>
